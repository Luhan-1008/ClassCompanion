...
interface GroupMemberDao {
    ...
    @Query("UPDATE group_members SET status = :status, invitedBy = :invitedBy WHERE groupId = :groupId AND userId = :userId")
    suspend fun updateMemberStatus(groupId: Int, userId: Int, status: MemberStatus, invitedBy: Int?)
    
    @Query("DELETE FROM group_members WHERE groupId = :groupId")
    suspend fun deleteMembersByGroup(groupId: Int)
}
